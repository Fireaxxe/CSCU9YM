;; Starter file for CSC9YM practical 3 (Doing experiments with BehaviorSpace)
...
to setup ;; called once, to create the landscape
  clear-all
  make-hills
end

to setup-run ;; called before each model run
  ask patches [ set visited? false] ;; reset patch variables
  clear-ticks ;; clear the tick counter
  clear-turtles ;; remove all turtles
  clear-drawing ;; removes lines drawn by turtles
  clear-all-plots ;; clears all plots
  clear-output ;; clears any output text (there is one)
  make-butterflies ;; create and initialise the butterflies
  reset-ticks ;; restart tick counter at 0
end


;;
;; load the landscape data from a file
;;
to make-hills
...
;;
;; Create the butterflies, set them all on the same patch, set down their pens, and remember their start patch
;;
to make-butterflies
  create-turtles num-butterflies
  [
    set size 3
    setxy 85 95
    pen-down
...
@#$#@#$#@
##OVERVIEW
This model is based on Pe’er et al. (2005). This ODD description is taken from Section 3.4 of Railsback and Grimm (2012). 
