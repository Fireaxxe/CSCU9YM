globals [
  ...
  damaged-car     ; a broken car that blocks a lane
]
...
to setup
  ;stop-inspecting selected-car
  clear-all
  set-default-shape turtles "car"
  draw-road
  create-or-remove-cars
  set selected-car one-of turtles
  inspect  selected-car
  set damaged-car one-of turtles
  ask selected-car [ set color lime ]
  ask damaged-car [set color red ]
  reset-ticks
end
...
to go
  create-or-remove-cars
  ask [ other turtles ] of damaged-car [ move-forward ]
  ask damaged-car [break-down-car]
  ask turtles with [ patience <= 0 ] [ choose-new-lane ]
  ask turtles with [ ycor != target-lane ] [ move-to-target-lane ]
  tick
  if ticks mod 250 = 0 [
    ask damaged-car [ set color blue ]
    set damaged-car one-of turtles
    ask damaged-car [ set color red ]
    ask damaged-car [ break-down-car ]
  ]
...
to select-car
  ...
  if mouse-down? [
    ...
    if any? turtles-on patch mx my [
      ...
      inspect  one-of turtles-on patch mx my
    ]
  ]
end
...
"" ""
PENS
"average" 1.0 0 -8630108 true "" "plot mean [ speed ] of turtles"
"max" 1.0 0 -13345367 true "" "plot max [ speed ] of turtles"
"min" 1.0 0 -2674135 true "" "plot min [ speed ] of turtles"
"selected-car" 1.0 0 -13840069 true "" "plot [ speed ] of selected-car"
...
"" ""
PENS
"average" 1.0 0 -8630108 true "" "plot mean [ patience ] of turtles"
"max" 1.0 0 -13345367 true "" "plot max [ patience ] of turtles"
"min" 1.0 0 -2674135 true "" "plot min [ patience ] of turtles"
"selected car" 1.0 0 -13840069 true "" "plot [patience] of selected-car"
...
As in the Traffic Basic model, traffic may slow down and jam without any centralized cause. Drivers may respond by changing directions, but often this does not solve the problem.

## STUDY

The adjustment we made to this model is that we have a slider so that the user can change the number of lanes in the model. Although it is not a change for the agent, it shows clearly that there is still traffic jam even if we have more lanes.

The agent-based change in this model is a car which randomly breaks down during a traffic jam. The car which breaks is red colored until it is fixed, then turns blue again and fuses into the rest of the flow. For a broken car, the same rules apply with traffic: if a car rises behind a broken car, it should shift into another lane to get it around. However, the traffic conditions are getting worse with this added layer. This is a valuable technique because it can also be observed in everyday life.

Through studying the models we learned how minor code changes can have a significant impact on the larger system. For example, if you add a semi-colon to the slow down car command (comment out a line of code), the traffic jam will be fixed because the cars have an adaptive cruise control. In this case, one character (;) changes the nature of the agents in the complex system completely.
...
- the maximum speed of any car - BLUE
- the minimum speed of any car - RED
- the average speed of all cars - VIOLET
- the speed of the selected car - GREEN
...
@#$#@#$#@
<experiments>
  <experiment name="Allv2" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>[speed] of selected-car</metric>
    <steppedValueSet variable="number-of-lanes" first="1" step="1" last="8"/>
    <steppedValueSet variable="max-patience" first="1" step="20" last="100"/>
    <steppedValueSet variable="acceleration" first="0.002" step="0.002" last="0.01"/>
    <steppedValueSet variable="number-of-cars" first="1" step="10" last="200"/>
    <steppedValueSet variable="deceleration" first="0.02" step="0.02" last="0.1"/>
  </experiment>
  <experiment name="Allv3" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>[speed] of selected-car</metric>
    <steppedValueSet variable="number-of-lanes" first="1" step="1" last="8"/>
